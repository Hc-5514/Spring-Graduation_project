...
class UserServiceTest {
    ...
    public void getUserInfoDto_성공() throws Exception {
        ...
        assertThat(userInfoDto.getUserSubscribeCount()).isEqualTo(0);
        assertThat(userInfoDto.getUserSubscriberCount()).isEqualTo(0);
        assertThat(userInfoDto.isLoginUser()).isEqualTo(true);
        assertThat(userInfoDto.isSubscribe()).isEqualTo(false);
    }

    @Test
    public void getUserInfoDto_유저찾기_실패() throws Exception {
        //given
        given(userRepository.findById(any())).willReturn(java.util.Optional.ofNullable(null));
